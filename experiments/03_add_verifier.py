...
from smart_llm_v2.agents.provider_factory import build_json_planner, build_plan_verifier
...
def main() -> None:
    ...
    planner = build_json_planner(
        profile,
        max_tokens=args.max_tokens,
        temperature=args.temperature,
    )
    verifier = build_plan_verifier(
        profile,
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--verifier-max-tokens", type=int)
...
def resolve_output_dir(output_dir: Path | None) -> Path:
    ...
    return Path("results") / f"{timestamp}-verifier"
...
def write_results(
    *,
    ...
    config = {
        ...
        "verifier_provider": profile.provider.value,
        ...
        json.dumps(
            {
                ...
                "verification_rejection_count": sum(
                    bool(task_run.verification_issues) for task_run in task_runs
                ),
            },
            indent=2,
            sort_keys=True,
...
def task_run_payload(task_run: TaskRunResult) -> dict[str, object]:
    return {
        ...
        "verifier_provider": task_run.verifier_provider,
        "verifier_model": task_run.verifier_model,
        "verifier_usage": dict(task_run.verifier_usage or {}),
        "verification_issues": [
            issue.to_payload() for issue in task_run.verification_issues
        ],
