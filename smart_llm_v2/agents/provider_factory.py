...
from smart_llm_v2.agents.verifier import PlanVerifier, SemanticVerifierClient
...
def build_semantic_verifier_client(
    profile: ModelProfile,
    *,
    max_tokens: int | None = None,
    temperature: float | None = None,
    ...
    if profile.transport is Transport.ANTHROPIC_MESSAGES:
        return AnthropicSemanticVerifierClient(
            model=profile.model,
            max_tokens=resolved_max_tokens,
            temperature=resolved_temperature,
            top_p=profile.top_p,
            thinking_budget_tokens=profile.thinking_budget_tokens,
            strict=profile.tool_strict,
            vision_enabled=profile.vision_enabled,
            client=client,
        )

    if profile.transport is Transport.OPENAI_CHAT_COMPLETIONS:
        return OpenAICompatibleSemanticVerifierClient(
            provider=profile.provider.value,
            model=profile.model,
            api_key_env_var=profile.api_key_env_var,
            base_url=profile.base_url,
...
def build_plan_verifier(
    ...
    *,
    ...
    return PlanVerifier(
        semantic_client=build_semantic_verifier_client(
            profile,
            max_tokens=max_tokens,
            temperature=temperature,
            client=client,
